class GenerateSalaryPaymentsJob < CustomJob
  ...
  def perform(_community_id: nil, payment_period_expense_id: nil, period_expense_id: nil, _message: I18n.t('jobs.generate_salary_payments_job'))
    ...
    period_expense = period_expense_id.present? ? community.period_expenses.find(period_expense_id) : nil
    ...
    salary_payments = employees.map do |employee|
    ...
    salary_payments.each do |salary_payment|
      Remuneration::SalaryPayments::CalculateSalary.call(
        salary_payment: salary_payment, on_creation: true
      )
    end
  end
end

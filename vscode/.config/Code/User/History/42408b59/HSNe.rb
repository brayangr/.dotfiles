...
RSpec.describe SalaryPayments::SalaryPaymentDirector do
  describe '#build' do
    let!(:community) { create(:chilean_community, :with_initial_period, ccaf: 'Los Andes') }
    let!(:period_expense) { community.period_expenses.first }

    context 'when called with a full time employee without draft for the payment period' do
      let!(:employee) { create(:full_time_employee, community: community) }

      context 'when there is previous salary payment' do
        ...
        let!(:discount) { create(:salary_additional_info, :discount, salary_payment_id: last_salary_payment.id) }
        let!(:advance) { create(:advance, community: community, employee: employee, period_expense: period_expense) }
        let!(:social_credit_fee) do
          social_credit = create(:social_credit, start_period_expense: period_expense, employee: employee,
                                                supplier: community.ccaf)
          create(:social_credit_fee, social_credit: social_credit, period_expense: period_expense)
        end
        ...
        it 'should copy the discounts data' do
          ...
          expect(salary_payment.advance).to eq(advance.price)
          expect(salary_payment.ccaf).to eq(social_credit_fee.price)

          expect(salary_payment.salary_additional_infos.select(&:discount).count).to eq(1)

          new_discount = salary_payment.salary_additional_infos.select(&:discount).first

          expect(new_discount.name).to eq(discount.name)
          expect(new_discount.value).to eq(discount.value)
        end
      ...
      context 'when there are no previous salary payment' do
        ...
        it 'should copy the identification data' do
          ...
          expect(salary_payment.payment_period_expense_id).to be(period_expense.id)
          expect(salary_payment.period_expense_id).to be(period_expense.id)
          expect(salary_payment.aliquot_id).to be(nil)
        end
        ...
        it 'should set all other fields with default values' do
          salary_payment = described_class.new(
            employee: employee,
            payment_period_expense: period_expense,
            period_expense: period_expense
          ...
          new_salary_payment = SalaryPayment.new

          expect(salary_payment.allocation_tool_wear).to eq(new_salary_payment.allocation_tool_wear)
          expect(salary_payment.lost_cash_allocation).to eq(new_salary_payment.lost_cash_allocation)
          expect(salary_payment.advance_gratifications).to eq(new_salary_payment.advance_gratifications)
          expect(salary_payment.anual_gratifications).to eq(new_salary_payment.anual_gratifications)
          expect(salary_payment.bono_responsabilidad).to eq(new_salary_payment.bono_responsabilidad)
          expect(salary_payment.salary_additional_infos).to eq(new_salary_payment.salary_additional_infos)
          expect(salary_payment.union_fee).to eq(new_salary_payment.union_fee)
          expect(salary_payment.legal_holds).to eq(new_salary_payment.legal_holds)
          expect(salary_payment.advance).to eq(new_salary_payment.advance)
          expect(salary_payment.ccaf).to eq(new_salary_payment.ccaf)
          expect(salary_payment.salary_additional_infos).to eq(new_salary_payment.salary_additional_infos)
          expect(salary_payment.apv).to eq(new_salary_payment.apv)
          expect(salary_payment.cotizacion_empleador_apvc).to eq(new_salary_payment.cotizacion_empleador_apvc)
          expect(salary_payment.cotizacion_trabajador_apvc).to eq(new_salary_payment.cotizacion_trabajador_apvc)
          expect(salary_payment.spouse).to eq(new_salary_payment.spouse)
          expect(salary_payment.spouse_voluntary_amount).to eq(new_salary_payment.spouse_voluntary_amount)
          expect(salary_payment.spouse_periods_number).to eq(new_salary_payment.spouse_periods_number)
          expect(salary_payment.spouse_capitalizacion_voluntaria).to eq(new_salary_payment.spouse_capitalizacion_voluntaria)
          expect(salary_payment.employee_protection_law).to eq(new_salary_payment.employee_protection_law)
          expect(salary_payment.protection_law_code).to eq(new_salary_payment.protection_law_code)
          expect(salary_payment.suspension_or_reduction_days).to eq(new_salary_payment.suspension_or_reduction_days)
          expect(salary_payment.reduction_percentage).to eq(new_salary_payment.reduction_percentage)
          expect(salary_payment.afc_informed_rent).to eq(new_salary_payment.afc_informed_rent)
          expect(salary_payment.adjust_by_rounding).to eq(new_salary_payment.adjust_by_rounding)
          expect(salary_payment.salary_additional_infos.size).to eq(0)
        end
      end
    end
  end

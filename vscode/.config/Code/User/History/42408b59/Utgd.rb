    ...
    context 'when called with a employee without draft for the payment period' do
      ...
      context 'when there are a previous salary payment' do
        ...
        it 'should copy the bonus data' do
          ...
          byebug
          expect(salary_payment.salary_additional_infos.where(discount: false).count).to eq(1)

          new_bonus = salary_payment.salary_additional_infos.where(discount: false).first

  ...
  describe 'Successful Cases' do
    context 'when filter without headers' do
      ...
      it 'return full names' do
        ...
        expect(response.select { |property_info| property_info['propertyId'] == random_property.id.to_s }.first['fullUserName'].to_lower).to eq(random_property.users.first.full_name)
      end

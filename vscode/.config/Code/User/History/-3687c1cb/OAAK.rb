  ...
  describe 'Successful Cases' do
    context 'when filter without headers' do
      ...
      it 'return full names' do
        random_property = properties.sample
        random_property.update(name: 'aaaa')

        expect(response.select { |property_info| property_info['propertyId'] == random_property.id.to_s }.first['fullUserName']).to eq(random_property.users.first.full_name)
      end

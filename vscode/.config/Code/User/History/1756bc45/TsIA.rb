      ...
      context 'when the community have remuneration package' do
        ...
        context 'when delay is true' do
          it 'should enqueue GenerateRecurrentAdvancesJob' do
            expect { community.get_open_period_expense.notify_close_3(true, true, user, Time.now) }.to have_enqueued_job(GenerateRecurrentAdvancesJob)
          end
        end

        context 'when delay is false' do
          it 'should call generate_recurrent_advances' do
            expect_any_instance_of(PeriodExpense).to receive(:generate_recurrent_advances)
            community.get_open_period_expense.notify_close_3(false, true, user, Time.now)
          end
        end
      end
      ...
      context 'when the community does not have remuneration package' do
        context 'when delay is true' do
          it 'should not enqueue GenerateRecurrentAdvancesJob' do
            expect { community.get_open_period_expense.notify_close_3(true, true, user, Time.now) }.to_not have_enqueued_job(GenerateRecurrentAdvancesJob)
          end
        end

        context 'when delay is false' do
          it 'should not call generate_recurrent_advances' do
            expect_any_instance_of(PeriodExpense).to_not receive(:generate_recurrent_advances)
            community.get_open_period_expense.notify_close_3(false, true, user, Time.now)
          end
        end
      end

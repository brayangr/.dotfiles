class CommunitiesController < ApplicationController
  ...
  def edit
    ...
    @initial_balances = @bank_accounts.map(&:first_bank_transaction)
    ...
    respond_to do |format|
      format.xlsx do
        if period_expense.common_expenses.count > 180
          ...
          document_excel = ExcelGenerator::CollectionSheetReport.new(community: current_community).call
          send_data document_excel.to_stream.read, filename: I18n.t('excels.billing_sheet.file_name', period_expense: period_expense, current_community: current_community), type: 'application/vnd.openxmlformates-officedocument.spreadsheetml.sheet'
        end
      end
    end
  end

...
RSpec.describe Remuneration::SalaryPaymentDraftsController, type: :controller do
  ...
  describe 'POST /remuneraciones/libro_liquidaciones/:id/reset' do
    before do
      login(user, community: community)
    end
  ...
  describe 'POST /remuneraciones/libro_liquidaciones/bulk_reset' do
    before do
      login(user, community: community)
    end

    let(:params) { { tab: :worked_days } }

    context 'when the user is super admin' do
      let(:user) { create(:user_superadmin) }

      it 'should return status 200' do
        post :bulk_reset, params: params
    ...
    describe 'GET /remuneraciones/libro_liquidaciones/save' do
      ...
      context 'when tab is worked_days' do
        ...
        it 'redirects to worked_days path' do
          get :save, params: { tab: :worked_days }

          expect(response.status).to redirect_to(remuneration_salary_payment_drafts_path(tab: :worked_days))
        end

        it 'includes the correct flash message' do
          get :save

          expect(response.flash[:notice]).to eq(I18n.t('views.remunerations.salary_payment_drafts.success'))
        end

        context 'when filters are applied' do
          it 'redirects with the filters' do
            params = { month: 10, year: 2023, employee_finder: 'test', tab: :worked_days }
            get :save, params: params

            expect(response.status).to redirect_to(remuneration_salary_payment_drafts_path(params))
          end
        end
      end
    end

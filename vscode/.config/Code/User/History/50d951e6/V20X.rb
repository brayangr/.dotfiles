class DateSelectorComponent < ViewComponent::Base
  def initialize(**params)
    ...
    @id = params[:id] || 'date'
    @default_value = params[:default_value].present? ? params[:default_value].to_date : Date.today
  ...
  def prepare_calendar_grid
    start_date = @default_value.beginning_of_month
    end_date = @default_value.end_of_month
    ...
    (- previous_days..days_of_month + post_days).map do |day|
      ...
      { day: date.day, month: date.month, year: date.year, current_month: date.month == @default_value.month, selected: same_date(@default_value, date), date: date }
    end
  end

FactoryBot.define do
  factory :salary_payment do
    ...
    trait :with_spouse_data do
      spouse { true }
      spouse_voluntary_amount { Faker::Number.between(from: 10, to: 20) }
      spouse_periods_number { Faker::Number.between(from: 10, to: 20) }
      spouse_capitalizacion_voluntaria { Faker::Number.between(from: 10, to: 20) }
    end
    ...
    trait :with_adjust_by_rounding do
      adjust_by_rounding { true }
    end

    trait :with_bonuses do
      before(:create) do |salary_payment|
        salary_payment.advance_gratifications = Faker::Number.between(from: 1, to: 20)
        salary_payment.anual_gratifications = Faker::Number.between(from: 1, to: 20)
        salary_payment.bono_responsabilidad = Faker::Number.between(from: 1, to: 20)
      end
    end

    trait :with_discounts do
      legal_holds { Faker::Number.between(from: 1, to: 20) }
      union_fee { Faker::Number.between(from: 1, to: 20) }
    end
  end

...
RSpec.describe Remuneration::SalaryPaymentDrafts::BulkResetResponse do
  ...
  describe 'when tab is :discounts' do
    ...
    it 'should delete all associated discounts' do
      expect do
        call_service_object(service_object: described_class, instantiate_context: controller_context, params: params)
      end.to change { DiscountsDraft.where(reason: DiscountsDraft.reasons[:custom_discount]).count }.from(
        DiscountsDraft.where(reason: DiscountsDraft.reasons[:custom_discount]).count).to(0)
    end
    ...
    context 'with filters' do
      ...
      it 'should not delete discounts drafts that does not match the filters' do
        expect do
          call_service_object(service_object: described_class, instantiate_context: controller_context, params: params)
        end.to_not change { full_time_employee_salary_payment_draft.reload.discounts_drafts.merge(DiscountsDraft.another_discounts).count }
          .from(full_time_employee_salary_payment_draft.another_discounts_drafts.count)
      end

      it 'should reset extra hours from salary payment drafts that match the filters' do
        expect do
          call_service_object(service_object: described_class, instantiate_context: controller_context, params: params)
        end.to change { another_part_time_employee_salary_payment_draft.reload.discounts_drafts.merge(DiscountsDraft.another_discounts).count }
          .from(another_part_time_employee_salary_payment_draft.another_discounts_drafts.count).to(0)
      end

...
RSpec.describe Remuneration::SalaryPaymentDrafts::BulkResetResponse do
  ...
  describe 'when tab is :licenses' do
    ...
    it 'should not reset other values' do
      ...
      expect do
        call_service_object(service_object: described_class, instantiate_context: controller_context, params: params)
      end.not_to change { full_time_employee_salary_payment_draft.reload.extra_hour }.from(1)

      expect do
        call_service_object(service_object: described_class, instantiate_context: controller_context, params: params)
      end.not_to change { full_time_employee_salary_payment_draft.reload.extra_hour_2 }.from(1)

      expect do
        call_service_object(service_object: described_class, instantiate_context: controller_context, params: params)
      end.not_to change { full_time_employee_salary_payment_draft.reload.extra_hour_3 }.from(1)
    end

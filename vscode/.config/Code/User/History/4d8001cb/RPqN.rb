...
RSpec.describe Remuneration::SalaryPaymentDrafts::BulkResetResponse do
  ...
  describe 'when tab is :taxable_bonus' do
    let(:params) { { community: community, tab: :taxable_bonus, user: user } }

    it 'should destroy bonus_drafts for employees' do
      expect do
        call_service_object(service_object: described_class, instantiate_context: controller_context, params: params)
      end.to change { part_time_employee_salary_payment_draft.reload.bonus_drafts.count }.from(2).to(0)
        .and.to change { full_time_employee_salary_payment_draft.reload.bonus_drafts.count }.from(1).to(0)
    end
    ...
    it 'should not reset other values' do
      expect do
        ...
        .and.not_to change { full_time_employee_salary_payment_draft.reload.worked_days }.from(10)

      expect do
        call_service_object(service_object: described_class, instantiate_context: controller_context, params: params)
      end.not_to change { part_time_employee_salary_payment_draft.reload.bono_days }.from(8)
        .and.not_to change { full_time_employee_salary_payment_draft.reload.bono_days }.from(8)

      expect do
        call_service_object(service_object: described_class, instantiate_context: controller_context, params: params)
      end.not_to change { part_time_employee_salary_payment_draft.reload.extra_hour }.from(10)
        .and.not_to change { full_time_employee_salary_payment_draft.reload.extra_hour }.from(10)

      expect do
        call_service_object(service_object: described_class, instantiate_context: controller_context, params: params)
      end.not_to change { part_time_employee_salary_payment_draft.reload.extra_hour_2 }.from(8)
        .and.not_to change { full_time_employee_salary_payment_draft.reload.extra_hour_2 }.from(8)

      expect do
        call_service_object(service_object: described_class, instantiate_context: controller_context, params: params)
      end.not_to change { part_time_employee_salary_payment_draft.reload.extra_hour_3 }.from(6)
        .and.not_to change { full_time_employee_salary_payment_draft.reload.extra_hour_3 }.from(6)
    end
    ...
    it 'should set updater' do
      expect do
        ...
        .and.to change { full_time_employee_salary_payment_draft.reload.updater_id }.from(nil).to(user.id)
    end
    ...
    it 'Should instantiate redirection_path' do
      ...
      expect(controller_context.instance_variable_get(:@redirection_path)).to eq(
        remuneration_salary_payment_drafts_path(month: month, year: year, tab: :taxable_bonus)
      )
    end
    ...
    context 'with filters' do
      ...
      let(:params) { { community: community, tab: :taxable_bonus, user: user, month: period_expense.month, year: period_expense.year } }
      ...
      it 'Should instantiate redirection_path' do
        ...
        expect(controller_context.instance_variable_get(:@redirection_path)).to eq(
          remuneration_salary_payment_drafts_path(month: period_expense.month, year: period_expense.year, tab: :taxable_bonus)
        )
      end
    end
  end

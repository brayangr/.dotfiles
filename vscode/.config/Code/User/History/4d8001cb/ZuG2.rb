...
RSpec.describe Remuneration::SalaryPaymentDrafts::BulkResetResponse do
  ...
  describe 'when tab is :discounts_days' do
    ...
    it 'should delete all associated discounts' do
      expect do
        call_service_object(service_object: described_class, instantiate_context: controller_context, params: params)
      end.to change { DiscountsDraft.discount_days.count }.from(DiscountsDraft.discount_days.count).to(0)
    end
    ...
    context 'with filters' do
      ...
      it 'should not delete discounts drafts that does not match the filters' do
        expect do
          call_service_object(service_object: described_class, instantiate_context: controller_context, params: params)
        end.to_not change { full_time_employee_salary_payment_draft.reload.discounts_drafts.discount_days.count }
          .from(full_time_employee_salary_payment_draft.discounts_drafts.discount_days.count)
      end

      it 'should reset extra hours from salary payment drafts that match the filters' do
        expect do
          call_service_object(service_object: described_class, instantiate_context: controller_context, params: params)
        end.to change { another_part_time_employee_salary_payment_draft.reload.discounts_drafts.discount_days.count }
          .from(another_part_time_employee_salary_payment_draft.discounts_drafts.discount_days.count).to(0)
      end

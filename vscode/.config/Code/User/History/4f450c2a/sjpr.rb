class Abilities::AdminManager
  ...
  def grant_billing
    ...
    can :receipt, InvoicePaymentAssignment do |i_p_a|
      (@admin_communities_ids & i_p_a&.account&.communities.pluck(:id)).any?
    end
  ...
  def grant_bills
    ...
    if @is_admin || @permission_by_code['bills'].try(:edit?)
      ...
      if @current_community.country_code == 'MX'
        # MEXICO: facturar
        can %i[irs_all cancel_irs irs_global], MxCompany if @current_community.mx_company.present?
        can %i[create create_complement], MxCompany
  ...
  def grant_property_fines
    ...
    if @is_admin || @permission_by_code['property_fines'].try(:edit?)
      ...
      can [:update], PropertyFines::BulksController
    end
  end
  ...
  def grant_service_billings
    ...
    if (@is_admin || @permission_by_code['service_billings'].try(:edit?)) && !@current_integration&.import_bills?
      ...
      can [:create, :new, :edit], [Supplier, Category]
      can [:create, :new], [Provision, ProvisionPeriodExpense] if @user.admin?
      ...
      can %i[edit update destroy import], Provision if @user.admin?

      can %i[update edit], ProvisionPeriodExpense if @user.admin?

      can [:destroy], ProvisionPeriodExpense do |s|
        @communities_ids.include?(s.period_expense.community_id)
      end
  ...
  def grant_employees
    ...
    if @is_admin || @permission_by_code['employees'].try(:edit?)
      ...
      can [:update, :edit, :nullify, :preview_modal, :upload_document, :create_massive], SalaryPayment do |e|
        @communities_ids.include?(e.employee.community_id)
      end
      ...
      can %i[index create update reset bulk_reset save], SalaryPaymentDraft
    end
  end
  ...
  def grant_posts
    ...
    if @is_admin || @permission_by_code['posts'].try(:edit?)
      ...
      can %i[create], Asset
      can %i[silent_destroy], Asset do |s|
        @communities_ids.include?(s.community_id)
      end
    end
  ...
  def grant_common_spaces
    ...
    if @is_admin || @permission_by_code['common_spaces'].try(:edit?)
      can [:new, :create], CommonSpace
      can [:new, :create, :information_audits], Event
  ...
  def grant_guest_registry
    ...
    if @is_admin || @permission_by_code['guest_registry'].try(:edit?)
      ...
      can [:index, :update_property_user_status, :synchronize_community, :bulk_update_property_users_status], AccessControlController
    end
  end
  ...
  def grant_surveys
    ...
    if @is_admin || @permission_by_code['surveys'].try(:edit?)
      ...
      can [:edit, :update, :destroy, :publish, :publish_results, :close_early, :save_options_title, :preview], Survey do |s|
        @communities_ids.include?(s.community_id)
      end

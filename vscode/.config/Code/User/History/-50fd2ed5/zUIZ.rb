...
class SalaryPayment < ApplicationRecord
  ...
  enum protection_law_code: { 0: :suspension_acto_autoridad, 1: :suspension_pacto reduccion_jornada_laboral]

  def apvi_set
    APVI_ATTRIBUTE_NAMES.each do |attribute|
      next unless !read_attribute(attribute).to_i.zero? && salary.institucion_apvi == 'No Cotiza A.P.V.'
  ...
  def apvc_set
    APVC_ATTRIBUTE_NAMES.each do |attribute|
      next unless !read_attribute(attribute).to_i.zero? && salary.institucion_apvc == 'No Cotiza A.P.V.'
      errors.add(attribute, :not_set)
    end
  end
  ...
  def self.prepare_new(employee, period_expense)
    ...
    if last_salary_payment.present?
      ...
      salary_payment.apv                              = employee.active_salary.institucion_apvi == 'No Cotiza A.P.V.' ? 0 : last_salary_payment.apv
      salary_payment.cotizacion_empleador_apvc        = employee.active_salary.institucion_apvc == 'No Cotiza A.P.V.' ? 0 : last_salary_payment.cotizacion_empleador_apvc
      salary_payment.cotizacion_trabajador_apvc       = employee.active_salary.institucion_apvc == 'No Cotiza A.P.V.' ? 0 : last_salary_payment.cotizacion_trabajador_apvc

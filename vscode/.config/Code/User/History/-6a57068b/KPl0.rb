    ...
    class IndexResponse < StandardServiceObject
      ...
      def filter_employees
        ...
        params = [
          ...
          "concat_ws(employees.first_name, ' ', employees.father_last_name)",
          "concat_ws(employees.first_name, ' ', employees.father_last_name, ' ', employees.mother_last_name)",
          "concat_ws(employees.father_last_name, ' ', employees.mother_last_name, ' ', employees.first_name)",
          "concat_ws(employees.father_last_name, ' ', employees.mother_last_name)"
        ]
      ...
      def set_last_salary_payments
        @last_salary_payments = Remunerations::SalaryPaymentDraftsQueries.last_salary_payments(
      ...
      def set_bonus_factor
        ...
        salaries = Salary.where(id: salaries_ids)
        return @bonus_factors = salaries.map { |salary| [salary.id, 1] }.to_h unless previred.present?

        @bonus_factors = salaries.map { |salary| [salary.id, calculate_bonus_factor(salary: salary, previred: previred, payment_period_expense: @payment_period_expense)] }.to_h
      end
      ...
      def instantiate_variables
        ...
        @response.add_data(:bonus_factors, @bonus_factors, instantiable: true) if @tab == :bonus
      end
    end
  end
end

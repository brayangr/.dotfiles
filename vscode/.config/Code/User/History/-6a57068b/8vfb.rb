  ...
  module SalaryPaymentDrafts
    class IndexResponse < StandardServiceObject
      ...
      def set_last_salary_payments
        ...
        raw_query = <<~SQL
          select employees.id, salary_payments.ultimo_total_imponible_sin_licencia from employees
          ...
          where employees.id in (:employees})
        SQL

        query = ActiveRecord::Base.sanitize_sql([raw_query, { employees: @employees.ids.join(',') }])
        ActiveRecord::Base.connection.execute(query).each_value do |values|
          byebug
          @last_salary_payments[values[0]] = values[1]
          # salary_payment = employee.salary_payments.where(nullified: false, validated: true, dias_licencia: 0).joins(:payment_period_expense).order('period_expenses.period desc').first

          # last_salary_payment = salary_payment.present? ? salary_payment.total_imponible : 0

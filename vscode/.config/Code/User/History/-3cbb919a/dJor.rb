...
module Remuneration
  class SalaryPaymentsController < RemunerationApplicationController
    ...
    def create_massive
      ...
      payment_period_expense = current_community.get_period_expense(
        params[:payment_period_expense_month].to_i,
        params[:payment_period_expense_year].to_i
      )

      period_expense = nil

      if params[:generate_period_expense].to_i == 1
        period_expense = current_community.get_period_expense(
          params[:period_expense_month].to_i,
          params[:period_expense_year].to_i
        )
      end
      ...
      ), notice: t('views.remunerations.massive_salary_payments.success')
    end

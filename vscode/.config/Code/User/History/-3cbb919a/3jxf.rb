...
module Remuneration
  class SalaryPaymentsController < RemunerationApplicationController
    ...
    def create_massive
      ...
      payment_period_expense = current_community.period_expenses.where(period: payment_period)

      period = "#{params[:period_expense_year]}-#{params[:period_expense_month]}-01"
      period_expense = params[:generate_period_expense] == 1 ? current_community.period_expenses.where(period: period) : nil

module SalaryPayments
  ...
  class BaseBuilder
    ...
    def to_h
      byebug
      additional_info_data = build_additional_info_data

      {
        salary_payment: @salary_payment.attributes.compact,
        bonos: Hash[additional_info_data[:bonus].map.with_index { |info, i| [i, info.attributes.compact] }],
        discounts: Hash[additional_info_data[:discounts].map.with_index { |info, i| [i, info.attributes.compact] }]
      }
    end
    ...
    def build_additional_info_data
      ...
      @salary_payment.salary_additional_infos.each do |additional_info|
        if additional_info.discount
          data[:discounts] << additional_info
        else
          data[:bonus] << additional_info
        end
      end

...
class SalaryPaymentDraft < ApplicationRecord
  ...
  has_many :discounts_drafts, -> { where.not(reason: DiscountsDraft.reasons[:another_reason]) }, dependent: :destroy
  has_many :another_discounts_drafts, -> { where(reason: DiscountsDraft.reasons[:another_reason]) }, class_name: 'DiscountsDraft', dependent: :destroy

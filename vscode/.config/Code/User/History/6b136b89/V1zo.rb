...
class SalaryPaymentDraft < ApplicationRecord
  ...
  has_many :discounts_drafts, -> { where.not(reason: DiscountsDraft.reasons[:custom_discount]) }, dependent: :destroy
  has_many :another_discounts_drafts, -> { where(reason: DiscountsDraft.reasons[:custom_discount]) }, class_name: 'DiscountsDraft', dependent: :destroy
  ...
  accepts_nested_attributes_for :another_discounts_drafts, allow_destroy: true
  accepts_nested_attributes_for :bonus_drafts, allow_destroy: true

  def discounts_days_drafts
    discounts_drafts.where.not(reason: DiscountsDraft.reasons[:custom_discount])
  end

  def another_discounts_drafts
    discounts_drafts.where(reason: DiscountsDraft.reasons[:custom_discount])
  end
end

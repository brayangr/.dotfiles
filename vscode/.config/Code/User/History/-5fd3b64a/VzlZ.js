...
export default class extends Controller {
  ...
  static targets = [ 'form', 'daysInput', 'startDateInput',
                     'endDateInput', 'lastSalaryInput' ]

  connect() {
    console.log(this.lastSalaryPaymentValue)
  }
  submit(){
    // Add validations
    console.log(this.lastSalaryPaymentValue)
  ...
  setLastSalary() {
    if (this.lastSalaryInputTarget.value == '') {
      let new_payment_price = EL_NUEVO_VALOR
      let priceInput = document.getElementById('TU_ID_FAVORITO')
      AutoNumeric.getAutoNumericElement(this.lastSalaryInputTarget.get(0)).set(new_payment_price)

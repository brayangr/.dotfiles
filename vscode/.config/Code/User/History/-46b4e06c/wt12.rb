  ...
  class SalaryPaymentDraftsQueries
    def self.employees(community_id:, start_date:, end_date:)
      Employee
        ...
        .where('(finiquitos.end_date >= ? or finiquitos.id isnull) and finiquitos.nullified = false', start_date)
    ...
    def self.last_salary_payments(employee_ids:)
      query = <<~SQL
        left join lateral (
      ...
      Employee
        ...
        .select(:id, 'salary_payments.total_imponible as last_salary_payment')
        .index_by(&:id)
    end
  end
end

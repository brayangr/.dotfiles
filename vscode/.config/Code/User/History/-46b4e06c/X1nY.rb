module Remunerations
  class SalaryPaymentDraftsQueries
    ...
    def self.salary_payment_drafts(employees:, payment_period_expense_id:)
      SalaryPaymentDraft
        .where(payment_period_expense_id: payment_period_expense_id, salary_id: employees.map(&:salary_id))
        .index_by(&:salary_id)
    end
  end
end

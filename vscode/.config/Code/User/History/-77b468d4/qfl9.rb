...
class DiscountsDraft < ApplicationRecord
  ...
  scope :discounts_days, -> { where.not(reason: DiscountsDraft.reasons[:custom_discount]) }
  scope :another_discounts, -> { where(reason: DiscountsDraft.reasons[:custom_discount]) }
end

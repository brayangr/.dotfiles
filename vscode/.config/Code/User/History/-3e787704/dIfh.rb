DefaultInit::Application.routes.draw do
  ...
  resources :access_control, only: %i[index] do
    member do
      put :update_property_user_status
    end

    collection do
      post :synchronize_community
      put  :bulk_update_property_users_status
    end
  end
  ...
  resources :property_fines, path: 'cargos' do
    collection do
      ...
      get 'generate_excel'
      get 'bulks' => 'property_fines/bulks#new', as: :new_bulks
      put 'bulks' => 'property_fines/bulks#update'
    end
  ...
  resources :events, path: 'eventos', only: [:index] do
    member do
      get 'information_audits'
    end
  end
  ...
  resources :invoices, only: %i[show index], path: 'facturas' do
    collection do
      ...
      get 'generate_excel', constraints: { format: :xlsx }
    end
    member do
      get 'pdf'
    end
  ...
  resources :invoice_payment_assignments, only: [], path: 'asignaciones' do
    member do
      get 'recibo', to: 'invoice_payment_assignments#receipt', as: 'receipt'
    end
  end
  ...
  namespace :admin do
    ...
    resources :invoice_payment_assignments do
      member do
        get 'receipt'
      end
    end
  ...
  namespace :async do
    defaults layout: false do
      ...
      namespace :remunerations do
        resources :salary_payment_drafts, only: %i[index]
      end

  ...
  module SalaryPaymentDrafts
    class BulkResetResponse < IndexResponse
      ...
      def call
        ...
        when :licenses
          reset_licenses(@salary_payment_drafts)
        when :discounts_days
          reset_discounts_days(@salary_payment_drafts)
        when :discounts
          reset_discounts(@salary_payment_drafts)
        when :bonus
          reset_bonus(@salary_payment_drafts)
        end
      ...
      def reset_extra_hours(salary_payment_drafts)
        ...
        reset!(data, %i[extra_hour extra_hour_2 extra_hour_3 updater_id])
      end

      def reset_licenses(salary_payment_drafts)
        data = prepare_data(salary_payment_drafts) do |salary_payment_draft|
          salary_payment_draft.license_drafts.destroy_all
        end

        reset!(data, %i[updater_id])
      end

      def reset_discounts_days(salary_payment_drafts)
        data = prepare_data(salary_payment_drafts) do |salary_payment_draft|
          salary_payment_draft.discounts_drafts.discounts_days.destroy_all
        end

        reset!(data, %i[updater_id])
      end

      def reset_discounts(salary_payment_drafts)
        data = prepare_data(salary_payment_drafts) do |salary_payment_draft|
          salary_payment_draft.discounts_drafts.another_discounts.destroy_all
        end

        reset!(data, %i[updater_id])
      end

      def reset_bonus(salary_payment_drafts)
        data = prepare_data(salary_payment_drafts) do |salary_payment_draft|
          salary_payment_draft.bonus_drafts.destroy_all
        end

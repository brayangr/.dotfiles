  ...
  describe 'Should instantiate expected variables in controller' do
    ...
    describe 'Should return expected records' do
      it 'Should return employee with ' do
        ...
        salary = part_time_employee.active_salary

        expect(response.data[:employee].id).to be_eql(part_time_employee.id)
        expect(response.data[:salaries]).to include({ part_time_employee.id => salary })

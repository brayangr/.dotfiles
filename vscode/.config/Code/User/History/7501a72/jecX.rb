module Remuneration
  module SalaryPaymentDrafts
    class ResetResponse < BaseResponse
      ...
      def reset_discounts_days
        @salary_payment_draft.update(updater_id: @user.id)
        @salary_payment_draft.discounts_drafts.merge(DiscountsDraft.discount_days).destroy_all
      end

      def reset_discounts
        @salary_payment_draft.update(updater_id: @user.id)
        @salary_payment_draft.discounts_drafts.merge(DiscountsDraft.another_discounts).destroy_all
      end

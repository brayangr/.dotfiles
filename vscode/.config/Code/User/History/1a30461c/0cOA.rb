...
class FinkokResponse < ApplicationRecord
  ...
  enum complement_status: { no_complement: 0, processing_complement: 1, complement_success: 2, complement_failed: 3, complement_in_queue: 4, cancelling_complement: 5, cancel_complement_failed: 6 }
  ...
  def cancel(mx_company, motivo = '02', folio_sustitucion = '')
    ...
    when '201', '202'
      ...
      status = Constants::FinkokResponse::CANCEL_ERRORS[response[:estatus_uuid]]
      response[:estatus_cancelacion] = status
      finkok_parent.update(complement_status: :cancel_complement_failed)
      Rollbar.error(StandardError.new("#{response[:estatus_uuid]} - #{status}"))
    end

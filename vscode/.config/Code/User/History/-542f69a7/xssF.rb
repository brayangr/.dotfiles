    ...
    unless undid
      if delay
        ...
        if remuneration_enabled?(community)
          GenerateRecurrentAdvancesJob.perform_later(_community_id: self.community_id, period_expense_id: self.id, _message: I18n.t('jobs.generate_recurrent_advances_job'), closing_log_id: closing_log_id)
        end
      else

  ...
  def notify_close_3(delay = true, notify = true, current_user = nil, new_bt_date = nil, closing_log_id = nil)
    ...
    unless undid
      if delay
        ...
        # TODO: Do not enqueue if the community does not have remuneration package
        if community.active_community_packages.exists?(package_type: 'RM') && community.get_setting_value('remuneration').positive?
        ...
        # TODO: Do not enqueue if the community does not have remuneration package
        if community.active_community_packages.exists?(package_type: 'RM') && current.get_setting_value('remuneration').positive?
          generate_recurrent_advances
        end
      end
    end
